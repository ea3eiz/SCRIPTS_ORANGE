...
salto=`grep -n -m 1 -c '\<OscOffset\>' ~/MMDVMHost/MMDVMPLUS.ini`
if [ $salto = 0 ]; then
echo "\33[1;31mEsta versión MMDVMHost no trae este parámetro"
else
salto=`grep -n -m 1 '\<OscOffset\>' ~/MMDVMHost/MMDVMPLUS.ini`
...
letra=p
linea2=$linea$letra
var200= sed -n $linea2  ~/MMDVMHost/MMDVMPLUS.ini;
...
echo "Valor actual del Master: \33[1;33m${master#*=}\33[1;37m"
                      read -p 'DMR+=eamaster04.xreflector.es  ó  eaglobalmaster.xreflector.es: ' master1
                          actualizar=S 
                          case $actualizar in
                    [sS]* ) echo ""
...
echo "Valor  actual  del  scOffset : \33[1;33m${salto#*=}\33[1;37m"
                         ...
                         if [ $largo = 3 ]
                          ...
                          case $actualizar in
			  [sS]* ) echo ""
                          sed -i "$linea OscOffset=$of1" ~/MMDVMHost/MMDVMPLUS.ini
			  break;;
			  [nN]* ) echo ""
			  break;;
esac
...
echo -n "Valor  actual  del \33[1;33m${var200#*=}\33[1;37m"
                          var100= sed -n $linea2  ~/MMDVMHost/MMDVMPLUS.ini;
                          ...
                          sed -i "$linea4 Enable=$dmrac1" ~/MMDVMHost/MMDVMPLUS.ini
			              break;;
			              [nN]* ) echo ""
			              break;;
esac
