...
mode=`grep -n -m 1 "^Port=" ~/MMDVMHost/MMDVMDMR2NXDN.ini`
buscar=":"
caracteres=`expr index $mode $buscar`
caracteres_linea=`expr $caracteres - 1`
numero_linea_port=`expr substr $mode 1 $caracteres_linea`
mode=$(awk "NR==$numero_linea_port" ~/MMDVMHost/MMDVMDMR2NXDN.ini)
echo "$mode"
...
# j) POCSAG Enable=
var=`grep -n -m 1 "\[POCSAG\]" ~/MMDVMHost/MMDVMDMR2NXDN.ini`
buscar=":"
largo_linea=`expr index $var $buscar`
largo_linea=`expr $largo_linea - 1`
numero_linea=`expr substr $var 1 $largo_linea`
numero_linea=`expr $numero_linea + 1` # Se le suma 1 al número de linea
POCSAG=$(awk "NR==$numero_linea" ~/MMDVMHost/MMDVMDMR2NXDN.ini)
letra=c
...
echo ""
var1=`grep -n "\[DMR Network\]" ~/DMR2NXDN/DMR2NXDN.ini` 
var=`echo "$var1" | tr -d '[[:space:]]'`
buscar=":"
largo_linea=`expr index $var $buscar`
largo_linea=`expr $largo_linea - 1`
...
Local=$(awk "NR==$numero_linea" ~/DMR2NXDN/DMR2NXDN.ini)
letra=c
...
do
                          ...
                          letrac=c
                          numero_linea_port=$numero_linea_port$letrac
                          sed -i "$numero_linea_port Port=/dev/ttyACM0" ~/MMDVMHost/MMDVMDMR2NXDN.ini
			  break;;
			  [nN]* ) echo ""
			  break;;
esac
...
do
                          ...
                          letrac=c
                          numero_linea_port=$numero_linea_port$letrac
                          sed -i "$numero_linea_port Port=/dev/ttyACM1" ~/MMDVMHost/MMDVMDMR2NXDN.ini
			  break;;
			  [nN]* ) echo ""
			  break;;
esac
...
do
                          ...
                          letrac=c
                          numero_linea_port=$numero_linea_port$letrac
                          sed -i "$numero_linea_port Port=/dev/ttyUSB0" ~/MMDVMHost/MMDVMDMR2NXDN.ini
			  break;;
			  [nN]* ) echo ""
			  break;;
esac
...
echo "Valor  actual  del Id: \33[1;33m${idd#*=}\33[1;37m"
           	          read -p 'Introduce un ID válido ' miid
                          ...
                          sed -i "$linea_sed_10 Id=$miid" ~/DMR2NXDN/DMR2NXDN.ini
...
master1=`echo "$master1" | tr [:upper:] [:lower:]`
                          ...
                          sed -i "$linea_master Address=$master1" ~/MMDVMHost/MMDVMDMR2NXDN.ini
...
do
                          ...
                          case $actualizar in
                          [sS]* ) echo ""
                          sed -i "$linea_sed_POCSAG Enable=$POCSAG1" ~/MMDVMHost/MMDVMDMR2NXDN.ini
                          break;;
                          [nN]* ) echo ""
                          break;;
esac
