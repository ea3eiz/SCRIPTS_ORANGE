...
loc=`grep -n "^Location=" ~/MMDVMHost/MMDVMDSTAR.ini`
...
contenido_location=$(awk "NR==$numero_linea" ~/MMDVMHost/MMDVMDSTAR.ini)
...
mode=`grep -n -m 1 "^Port=" ~/MMDVMHost/MMDVMDSTAR.ini`
buscar=":"
caracteres=`expr index $mode $buscar`
caracteres_linea=`expr $caracteres - 1`
numero_linea_port=`expr substr $mode 1 $caracteres_linea`
mode=$(awk "NR==$numero_linea_port" ~/MMDVMHost/MMDVMDSTAR.ini)
echo "$mode"
...
dstar=`grep -n "\[D-Star\]" ~/MMDVMHost/MMDVMDSTAR.ini` # devuelve ejem: 74:Enable=1
buscar=":"
...
presentar_valo= sed -n $numero_linea_dstar_letrap  ~/MMDVMHost/MMDVMDSTAR.ini; #presenta el valor en pantalla
...
dmr=`grep -n "\[DMR\]" ~/MMDVMHost/MMDVMDSTAR.ini` # devuelve ejem: 74:Enable=1
buscar=":"
...
presentar_valor= sed -n $numero_linea_dmr_letrap  ~/MMDVMHost/MMDVMDSTAR.ini; #presenta el valor en pantalla
...
fusion=`grep -n "LowDeviation" ~/MMDVMHost/MMDVMDSTAR.ini` # devuelve ejem: 74:Enable=1
buscar=":"
...
presentar_valor= sed -n $numero_linea_fusion_letrap  ~/MMDVMHost/MMDVMDSTAR.ini; #presenta el valor en pantalla
...
presentar_valor= sed -n $numero_linea_p25_letrap  ~/MMDVMHost/MMDVMDSTAR.ini; #presenta el valor en pantalla
...
cw= sed -n "31p"  ~/MMDVMHost/MMDVMDSTAR.ini; #presenta el valor en pantalla
...
modehang=`grep -n -m 1 -c '\<RFModeHang\>' ~/MMDVMHost/MMDVMDSTAR.ini`
if [ $modehang = 0 ]; then
echo "\33[1;31mEsta versión MMDVMHost no trae este parámetro"
else
modehang=`grep -n -m 1 '\<RFModeHang\>' ~/MMDVMHost/MMDVMDSTAR.ini`
...
timeo=`grep -n -m 1 -c '\<Timeout\>' ~/MMDVMHost/MMDVMDSTAR.ini`
if [ $timeo = 0 ]; then
echo "\33[1;31mEsta versión MMDVMHost no trae este parámetro"
else
timeo=`grep -n -m 1 '\<Timeout\>' ~/MMDVMHost/MMDVMDSTAR.ini`
timeo1=`expr substr $timeo 3 30`
echo "$timeo1"
fi

echo -n "\33[1;36m  21)\33[0m Tipo Pantalla Display - \33[1;33m"
Display=`grep -n -m 1 -c '\<Display\>' ~/MMDVMHost/MMDVMDSTAR.ini`
if [ $Display = 0 ]; then
echo "\33[1;31mEsta versión MMDVMHost no trae este parámetro"
else
Display=`grep -n -m 1 '\<Display\>' ~/MMDVMHost/MMDVMDSTAR.ini`
Display1=`expr substr $Display 3 30`
echo -n "$Display1"
fi

var=`grep -n -m 1 "\[Nextion\]" ~/MMDVMHost/MMDVMDSTAR.ini`
#var1=`grep -m 1 "\[Nextion\]" ~/MMDVMHost/MMDVMDSTAR.ini`
buscar=":"
largo_linea=`expr index $var $buscar`
largo_linea=`expr $largo_linea - 1`
numero_linea=`expr substr $var 1 $largo_linea`
numero_linea=`expr $numero_linea + 2` # y le suma uno qudando coomo: (75)
MODEMNEXTION=$(awk "NR==$numero_linea" ~/MMDVMHost/MMDVMDSTAR.ini)
letra=c
...
ScreenLayout=`grep -n -m 1 -c '\<ScreenLayout\>' ~/MMDVMHost/MMDVMDSTAR.ini`
...
Brightness=$(awk "NR==$numero_linea" ~/MMDVMHost/MMDVMDSTAR.ini)
letra=c
linea_sed_Brightness=$numero_linea$letra
echo -n "  ${CIAN}23) ${GRIS}Brillo Display Nextion- ${AMARILLO}$Brightness"

# j) POCSAG Enable=
var=`grep -n -m 1 "\[POCSAG\]" ~/MMDVMHost/MMDVMDSTAR.ini`
buscar=":"
largo_linea=`expr index $var $buscar`
largo_linea=`expr $largo_linea - 1`
numero_linea=`expr substr $var 1 $largo_linea`
numero_linea=`expr $numero_linea + 1` # Se le suma 1 al número de linea
POCSAG=$(awk "NR==$numero_linea" ~/MMDVMHost/MMDVMDSTAR.ini)
letra=c
...
var300port= sed -n $linea22port  ~/MMDVMHost/MMDVMDSTAR.ini;
...
indi=$(awk "NR==2" ~/MMDVMHost/MMDVMDSTAR.ini)
...
frec=$(awk "NR==13" ~/MMDVMHost/MMDVMDSTAR.ini)
...
do
                          ...
                          case $actualizar in
                          [sS]* ) echo ""
                          sed -i "$numero_linea_letrac Location=$loc1" ~/MMDVMHost/MMDVMDSTAR.ini
                          break;;
                          [nN]* ) echo ""
                          break;;
esac
...
do
                          ...
                          sed -i "$numero_linea_port Port=/dev/ttyS3" ~/MMDVMHost/MMDVMDSTAR.ini
			  break;;
			  [nN]* ) echo ""
			  break;;
esac
...
do
                          ...
                          letrac=c
                          numero_linea_port=$numero_linea_port$letrac
                          sed -i "$numero_linea_port Port=/dev/ttyACM0" ~/MMDVMHost/MMDVMDSTAR.ini
			  break;;
			  [nN]* ) echo ""
			  break;;
esac
...
do
                          ...
                          letrac=c
                          numero_linea_port=$numero_linea_port$letrac
                          sed -i "$numero_linea_port Port=/dev/ttyACM1" ~/MMDVMHost/MMDVMDSTAR.ini
			  break;;
			  [nN]* ) echo ""
			  break;;
esac
...
do
                          ...
                          letrac=c
                          numero_linea_port=$numero_linea_port$letrac
                          sed -i "$numero_linea_port Port=/dev/ttyUSB0" ~/MMDVMHost/MMDVMDSTAR.ini
			  break;;
			  [nN]* ) echo ""
			  break;;
esac
...
ide=$(awk "NR==3" ~/MMDVMHost/MMDVMDSTAR.ini)
...
master=$(awk "NR==139" ~/MMDVMHost/MMDVMDSTAR.ini)
...
echo "Valor actual del Display=: \33[1;33m${Display1#*=}\33[1;37m"
                    ...
                    [sS]* ) echo ""
                    V=`echo "$V" | tr -d '[[:space:]]'`       
                          sed -i "$linea Display=$V" ~/MMDVMHost/MMDVMDSTAR.ini             
        break;;
        [nN]* ) echo ""
        break;;
esac
...
echo "Valor actual del ScreenLayout=: \33[1;33m${ScreenLayout1#*=}\33[1;37m"
                    ...
                    [sS]* ) echo ""
                    V=`echo "$V" | tr -d '[[:space:]]'`       
                          sed -i "$linea ScreenLayout=$V" ~/MMDVMHost/MMDVMDSTAR.ini             
        break;;
        [nN]* ) echo ""
        break;;
esac
...
do
                          ...
                          [sS]* ) echo ""
                          V=`echo "$V" | tr -d '[[:space:]]'`      
                          sed -i "$linea_sed_Brightness Brightness=$V" ~/MMDVMHost/MMDVMDSTAR.ini             
        break;;
        [nN]* ) echo ""
        break;;
esac
...
do
                          ...
                          presenta_valor= sed -n $numero_linea_dstar_letrap  ~/MMDVMHost/MMDVMDSTAR.ini;
                          read -p 'Desactivado=0 Activado=1:  '   dmrac1
                          actualizar=S 
                          case $actualizar in
                          [sS]* ) echo ""
                          sed -i "$numero_linea_dstar_letrac Enable=$dmrac1" ~/MMDVMHost/MMDVMDSTAR.ini
                          break;;
                          [nN]* ) echo ""
                          break;;
esac
...
do
                          ...
                          presenta_valor= sed -n $numero_linea_dmr_letrap  ~/MMDVMHost/MMDVMDSTAR.ini;
           	              read -p 'Desactivado=0 Activado=1: '   dmrac1
                          actualizar=S 
                          case $actualizar in
			                    [sS]* ) echo ""
                          sed -i "$numero_linea_dmr_letrac Enable=$dmrac1" ~/MMDVMHost/MMDVMDSTAR.ini
			                    break;;
			                    [nN]* ) echo ""
			                    break;;
esac
...
do
                          ...
                          presenta_valor= sed -n $numero_linea_fusion_letrap  ~/MMDVMHost/MMDVMDSTAR.ini;
                          read -p 'Desactivado=0 Activado=1:  '   dmrac1
                          actualizar=S 
                          case $actualizar in
                          [sS]* ) echo ""
                          sed -i "$numero_linea_fusion_letrac Enable=$dmrac1" ~/MMDVMHost/MMDVMDSTAR.ini
                          break;;
                          [nN]* ) echo ""
                          break;;
esac
...
do
                          ...
                          presenta_valor= sed -n $numero_linea_p25_letrap  ~/MMDVMHost/MMDVMDSTAR.ini;
                          read -p 'Desactivado=0 Activado=1: '   dmrac1
                          actualizar=S 
                          case $actualizar in
                          [sS]* ) echo ""
                          sed -i "$numero_linea_p25_letrac Enable=$dmrac1" ~/MMDVMHost/MMDVMDSTAR.ini
                          break;;
                          [nN]* ) echo ""
                          break;;
esac
...
echo "Valor actual del RFModeHang = : \33[1;33m${modehang1#*=}\33[1;37m"
                    ...
                    [sS]* ) echo ""
                    V=`echo "$V" | tr -d '[[:space:]]'`       
                          sed -i "$linea RFModeHang=$V" ~/MMDVMHost/MMDVMDSTAR.ini             
        break;;
        [nN]* ) echo ""
        break;;
esac
...
echo "Valor actual del Timeout = : \33[1;33m${timeo1#*=}\33[1;37m"
                    ...
                    [sS]* ) echo ""
                    V=`echo "$V" | tr -d '[[:space:]]'`       
                          sed -i "$linea Timeout=$V" ~/MMDVMHost/MMDVMDSTAR.ini             
        break;;
        [nN]* ) echo ""
        break;;
esac
...
do
                          ...
                          case $actualizar in
                          [sS]* ) echo ""
                          sed -i "$linea_sed_POCSAG Enable=$POCSAG1" ~/MMDVMHost/MMDVMDSTAR.ini
                          break;;
                          [nN]* ) echo ""
                          break;;
esac
...
do
			  ...
			  [sS]* ) echo ""
			   read -p 'Intruduce reflector DMR+ al que se conectara (ej:4370) ' opcion
                          ...
                          sed -i "$linea4 Options=StartRef=$opcion;RelinkTime=10;" ~/MMDVMHost/MMDVMDSTAR.ini
			  break;;
			  [nN]* ) echo ""
			  letra1=c
                          linea4=$linea33port$letra1
			  sed -i "$linea4 #Options=StartRef=4370;RelinkTime=10;" ~/MMDVMHost/MMDVMDSTAR.ini
			  break;;
esac
done;;
#27) echo ""
...
do
                        ...
                        sleep 3
                        sudo cp -f ~/MMDVMHost/MMDVMDSTAR.ini ~/MMDVMHost/MMDVMDSTAR.ini_copia
			                  break;;
			                  [nN]* ) echo ""
			                  break;;
esac
...
do
                        ...
                        sleep 3
                        sudo cp -f ~/MMDVMHost/MMDVMDSTAR.ini_copia ~/MMDVMHost/MMDVMDSTAR.ini
			                  break;;
			                  [nN]* ) echo ""
			                  break;;
esac
...
do
                        ...
                        sleep 3
                        sudo cp -f ~/MMDVMHost/MMDVMDSTAR.ini ~/MMDVMHost/MMDVMDSTAR.ini_copia2
			                  break;;
			                  [nN]* ) echo ""
			                  break;;
esac
...
do
                        ...
                        sleep 3
                        sudo cp -f ~/MMDVMHost/MMDVMDSTAR.ini_copia2 ~/MMDVMHost/MMDVMDSTAR.ini
			                  break;;
			                  [nN]* ) echo ""
			                  break;;
esac
...
do
                        ...
                        sleep 3
                        sudo cp -f ~/MMDVMHost/MMDVMDSTAR.ini ~/MMDVMHost/MMDVMDSTAR.ini_copia3
			                  break;;
			                  [nN]* ) echo ""
			                  break;;
esac
...
do
                        ...
                        sleep 3
                        sudo cp -f ~/MMDVMHost/MMDVMDSTAR.ini_copia3 ~/MMDVMHost/MMDVMDSTAR.ini
			                  break;;
			                  [nN]* ) echo ""
			                  break;;
esac
...
do
			...
			[sS]* ) echo ""
                        ...
                        sleep 3
                        sudo cp -f ~/MMDVMHost/MMDVM.ini_original ~/MMDVMHost/MMDVMDSTAR.ini
			break;;
			[nN]* ) echo ""
			break;;
esac
